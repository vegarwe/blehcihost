...
def get_peer_db(dev, conn_handle, start_handle = '\x01\x00'):
    ...
    while True:
        ...
        if not isinstance(pkt, hci.HciDataPkt):
            break
        findreturn = pkt.payload_pkt.payload_pkt
        if not isinstance(findreturn, hci.AttFindInformationResponse):
            if isinstance(findreturn, hci.AttErrorResponse):
                pass
            break
        t = findreturn.attributes[-1][0]
        t = ('%04x' % ((ord(t[1]) << 8) + ord(t[0]) + 1)).decode('hex')
        start_handle = t[1] + t[0]
        for attr in findreturn.attributes:
